...
function OutlineCard(props) {
    ...
    const likeNote = () => {
        if (cookieParser.getCookieByName('email')) {
            axios.put('http://192.168.0.7:8080:8080/like/note/' + props.cardContent.id + '/' + props.email, {}, {
                headers: {
    ...
    const unLikeNote = () => {
        if (cookieParser.getCookieByName('email')) {
            axios.put('http://192.168.0.7:8080:8080/unlike/note/' + props.cardContent.id + '/' + props.email, {}, {
                headers: {
                    'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                }
            }).then((res) => {
