...
const NoteEditTemplate = (props) => {
    ...
    useEffect(() => {
        ...
        if (note && props.mode == 'edit') {
            if (note?.type == 'reward') {
                ...
                axios.get(`http://192.168.0.7:8080:8080/post/${note?.postId}`)
                    .then(res => {
    ...
    const renameVersion = (index, name) => {
        ...
        axios.put(`http://192.168.0.7:8080:8080/note/${noteId}/${index}/${name}`, {}, {
            headers: {
    ...
    const infoSubmit = () => {
        ...
        if (props.mode == "edit") {
            ...
            axios.put(`http://192.168.0.7:8080:8080/note/${note.id}`, note, {
                headers: {
                    'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                }
            })
            ...
            let path = props.folderId ? `http://192.168.0.7:8080:8080/note/${email}/${props.folderId}` : `http://192.168.0.7:8080:8080/note/${email}`
            axios.post(path, NoteFormat, {
                ...
                .then(res => {
                    ...
                    axios.put(`http://192.168.0.7:8080:8080/note/${tempId}/0`, VersionFormat, {
                        headers: {
                            'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                        }
                    })
            ...
            axios.post(`http://192.168.0.7:8080:8080/post/reward/${props.postId}/${email}`, NoteFormat, {
                headers: {
                    'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                }
            })
    ...
    const setVersion = (index) => {
        axios.get(`http://192.168.0.7:8080:8080/note/${noteId}/${index}`)
            .then(res => {
                const defaultVersion = res.data.res
                defaultVersion.name = "default"
                defaultVersion.slug = "default"
                axios.put(`http://192.168.0.7:8080:8080/note/${noteId}/0`, defaultVersion, {
                    headers: {
                        'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                    }
                })
    ...
    const noteFinish = async () => {
        ...
        axios.put(`http://192.168.0.7:8080:8080/note/tag/wordSuggestion/${noteId}`, {}, {
            headers: {
                'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
            }
        })
            .then(res => {
                ...
                axios.get(`http://192.168.0.7:8080:8080/note/plagiarism/${noteId}`, {
                    headers: {
                        'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                    }
                })
    ...
    const tagSubmit = async () => {
        ...
        console.log("tagSelected", {tagSelected})
        axios.put(`http://192.168.0.7:8080:8080/note/tag/updateTags/${noteId}`, {"tags":tagSelected},  {
            headers: {
                'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
            }
        })
    ...
    const newVersion = (name) => {
        ...
        VersionFormat.content = [ContentFormat]
        axios.put(`http://192.168.0.7:8080:8080/note/${noteId}/${versionLength}`, VersionFormat, {
            headers: {
                'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
            }
        })
    ...
    const submitRewardNote = () => {
        axios.get(`http://192.168.0.7:8080:8080/note/${noteId}`)
            .then(res => {
                const tempNote = res.data.res
                tempNote.tag = tagSelected
                axios.put(`http://192.168.0.7:8080:8080/note/${noteId}`, tempNote, {
                    headers: {
                        'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                    }
                })
                    .then(res => {
                        axios.put(`http://192.168.0.7:8080:8080/note/submit/${noteId}`, {}, {
                            headers: {
                                'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                            }
                        })
