    ...
    useEffect(() => {
        async function getCollabById() {
            try {
                ...
                await axios.get('http://localhost:8080/search/post/'+props.keyword+'/'+ String(pageNumber - 1) + '/20?department='+(props.department?props.department:'')+'&subject='+(props.subject?props.subject:'')+'&haveCollaboration=' +true + '&sortBy=' + sortBy).then((res) => {
