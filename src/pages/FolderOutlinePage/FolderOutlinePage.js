...
function FolderOutlinePage(props) {
    ...
    useEffect(() => {
        ...
        async function getFolderById() {
            try {
                await axios.get('http://192.168.0.7:8080:8080/search/folder/' + String(props.pageNumber - 1) + '/20?keyword=' + (props.keyword ? props.keyword : '') + '&creator=' + (props.headerName ? props.headerName : '')).then((res) => {
