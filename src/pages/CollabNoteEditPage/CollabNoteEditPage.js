...
const CollabNoteEditPage = (props) => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://192.168.0.7:8080:8080/note/${props.noteId}`)
            .then(res => {
    ...
    useEffect(() => {
        ...
        if (newcomer.length != 0) {
            newcomer.map(userEmail => {
                axios.get(`http://192.168.0.7:8080:8080/user/${userEmail}`, {
                    headers: {
    ...
    */
    ...
    const infoSubmit = () => {
        ...
        axios.put(`http://192.168.0.7:8080:8080/note/${props.noteId}`, tempNote, {
            headers: {
                'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
            }
        })
    ...
    const setVersion = (index) => {
        axios.get(`http://192.168.0.7:8080:8080/note/${props.noteId}/${index}`)
            .then(res => {
                const defaultVersion = res.data.res
                defaultVersion.name = "default"
                defaultVersion.slug = "default"
                axios.put(`http://192.168.0.7:8080:8080/note/${props.noteId}/0`, defaultVersion, {
                    headers: {
                        'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                    }
                })
    ...
    const noteFinish = async () => {
        if (props.isManager) {
            axios.put(`http://192.168.0.7:8080:8080/note/tag/wordSuggestion/${note.id}`, {}, {
                headers: {
                    'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
                }
            })
    ...
    const newVersion = (name) => {
        ...
        axios.put(`http://192.168.0.7:8080:8080/note/${props.noteId}/${versionLength}`, VersionFormat, {
            headers: {
                'Authorization': 'Bearer ' + cookieParser.getCookieByName("token"),
            }
        })
