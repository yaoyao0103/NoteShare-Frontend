...
function MemberPage(props) {
    ...
    async function getFollowingNoteById(email) {
        try {
            await axios.get('http://192.168.0.7:8080:8080/note/following/' + email + '/' + String(props.pageNumber - 1) + '/10', {
                headers: {
    ...
    async function getRecommendNoteById() {
        try {
            ...
            await axios.get('http://192.168.0.7:8080:8080/note/hotNotes/' + String(props.pageNumber - 1) + '/10').then((res) => {
                setNote(oldArray => [...oldArray, res.data.res]);
                props.setLoading(false)
            });
